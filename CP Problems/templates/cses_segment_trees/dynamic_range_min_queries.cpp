...
long long build_tree(int root_idx, vector<ll>& tree, vector<int>& arr, const ll IDENTITY_VAL) {
  ...
  return tree[root_idx] = min(left, right);
}


// never call range_min_query_helper directly!
...
long long range_min_query_helper(int root_idx, int l, int r, vector<ll>& tree, vector<int>& arr, ll IDENTITY_VAL, int n) {
  ...
  // TODO: change operation
  return min(range_min_query_helper(root_idx * 2, l, r, tree, arr, IDENTITY_VAL, n), range_min_query_helper(root_idx * 2 + 1, l, r, tree, arr, IDENTITY_VAL, n));
}

long long range_min_query(int l, int r, vector<ll>& tree, vector<int>& arr, ll IDENTITY_VAL) {
  int n = upper_power_of_2(arr.size());
  return range_min_query_helper(1, l, r, tree, arr, IDENTITY_VAL, n);
}
...
ll update_helper(int arr_idx, ll new_val, int root_idx, vector<ll>& tree, int n, vector<int>& arr, int IDENTITY_VAL) {
  ...
  return tree[root_idx] = min(left_min, right_min);
}


void update(int arr_idx, ll new_val, vector<ll>& tree, vector<int>& arr, int IDENTITY_VAL)  {
  int n = upper_power_of_2(arr.size());
...
int main() {
 ...
 const int IDENTITY_VAL = INT_MAX;
 int n, q;
 cin >> n >> q;
 vector<int> arr(n);
 for (int i = 0; i < n; i++) {
 ...
 // create tree
 // always initialize tree size to nearest power of 2 of arr
 // tree's rooted at idx 1
int next_power_of_two = upper_power_of_2(n);
 vector<ll> tree(2 * next_power_of_two, IDENTITY_VAL);
 build_tree(1, tree, arr, IDENTITY_VAL);
//  for (int i : tree) cout << i << endl;
 while (q--) {
  int t, a, b;
  cin >> t >> a >> b;
  if (t == 1) {
    update(a - 1, b, tree, arr, IDENTITY_VAL);
  } else {
    cout << range_min_query(a - 1, b, tree, arr, IDENTITY_VAL) << '\n';
  }
 }
